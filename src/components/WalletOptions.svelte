...
>
  <div
    class="wallet-options-header flex justify-between items-center border-b-[1px] border-zilkroad-gray-border pb-5"
  >
  ...
  <div
    class="wallet-options-menu py-5 border-b-[1px] border-zilkroad-gray-border"
  >
    <ul>
      ...
      <li class='mb-[10px] last:mb-0'>
        ...
        >
          <DisconnectWallet />
          <span class='ml-[10px]'>Disconnect wallet</span
          ></button
        >
      </li>
    </ul>
  ...
  <div
    class="wallet-options-currencies py-5 border-b-[1px] border-zilkroad-gray-border grid grid-cols-2 gap-5"
  >
  ...
  <div class="wallet-options-convert-button pt-5">
    {#if showTokenSwap}
      <TokenSwap />
      <button
        class="text-white h-16 flex items-center p-5 w-full underline justify-center"
        on:click={toggleTokenSWAP}>Cancel swap</button
      >
    {/if}
    {#if !showTokenSwap}
      <button
        class="text-white h-16 flex items-center bg-zilkroad-gray-dark p-5 rounded-lg w-full"
        on:click={toggleTokenSWAP}
        ><Zil /><span class="ml-5">Convert Zil</span></button
      >
    {/if}
  </div>
</div>
