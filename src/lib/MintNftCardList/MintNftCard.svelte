...
<div
  class="grid grid-cols-4 lg:flex lg:h-24 bg-gray-900 rounded-lg"
  on:mouseenter={enter}
  on:mouseleave={leave}
>
  <div class="m-5 lg:m-0 col-span-4 flex">
    ...
    <div class="m-5 w-8/12">
      <p class="text-zilkroad-text-light">{collection.name}</p>
  ...
  <div
    class="my-5 lg:m-5 pl-5 lg:w-36 border-r-[1px] lg:border-l-[1px] border-gray-600"
  >
  ...
  <div
    class="mx-5 py-5 flex-grow col-span-4 lg:hidden border-t-[1px] border-gray-600"
  >
    <MintCount
      nftToMint={collection.id}
      count={collection.mintCount}
